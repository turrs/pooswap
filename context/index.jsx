...
export const ContextWrapper = ({ children }) => {
  const [tokenOneDecimals, setTokenOneDecimals] = useState(0);
  const [tokenTwoDecimals, setTokenTwoDecimals] = useState(0);
  const [contractTokenOne, setContractTokenOne] = useState();
  const [contractTokenTwo, setContractTokenTwo] = useState();
  const [tokenOneName, setTokenOneName] = useState("MATIC");
  const [tokenOneAmount, setTokenOneAmount] = useState(1);
  const [tokenTwoName, setTokenTwoName] = useState("USDT");
  const [tokenTwoAmount, setTokenTwoAmount] = useState();
  const [tokenOneImage, setTokenOneImage] = useState(
    "https://wallet-asset.matic.network/img/tokens/matic.svg"
  );
  const [tokenTwoImage, setTokenTwoImage] = useState(
